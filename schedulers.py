...
class SDXLCompatibleSchedulers(Enum):
    ...
    # Rectified Flow (flow matching) schedulers
    FlowMatchEuler = ("Flow Match Euler", FlowMatchEulerDiscreteScheduler, {})
    ...
    def create_instance(cls, name):
        for scheduler in cls:
            if scheduler.string_name == name:
                if scheduler.scheduler_class is FlowMatchEulerDiscreteScheduler:
                    return scheduler.scheduler_class(**scheduler.init_args)
